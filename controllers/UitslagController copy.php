...
class UitslagController extends Controller
{
    ...
    public function actionIndex($examenid="", $sortorder=1, $export=0) {
        ...
        // ";
        $sql="
            select s.naam naam, s.nummer studentid, s.klas klas, u.werkproces werkproces, '1' cijfer, u.cijfer cijfer2
            from uitslag u
        ...
        # this was used in the old version with subforms, this will still work.
        $sql="SELECT  s.naam,  f.werkproces, u.ready ready, COUNT(distinct g.formid) cnt
            FROM student s
            LEFT JOIN gesprek g ON s.id=g.studentid
            LEFT JOIN form f ON f.id = g.formid
            LEFT JOIN examen e ON e.id=f.examenid
            LEFT JOIN uitslag u ON u.studentid=s.id AND u.werkproces=f.werkproces AND u.examenid=:examenid
            WHERE (e.id=:examenid or e.id is null)
            AND (f.examenid=:examenid or f.examenid is null)
            AND s.actief=1
            GROUP BY 1,2,3
        ...
        foreach($uitslagen as $uitslag) {
            if ($uitslag['ready']==1) {
                $dataSet[$uitslag['naam']][$uitslag['werkproces']]['status']=3;
            }
        }
