            ...
            <?php
                ...
                foreach($dataSet as $naam => $value) {
                    ...
                    foreach($wp as $thisWp) { // Print Ready
                        echo "<td class=\"even\">"; 
                        if ( ($value[$thisWp]['status']??0)==$formWpCount[$thisWp] ) echo  Html::a( "<div class=\"text-success\"><b>".$value[$thisWp]['status']."</b></div>" , ['/uitslag/result', 'studentid'=>$value['studentid'], 'wp'=>$thisWp ] );
                        elseif ( ($value[$thisWp]['status']??0)==99 )  echo Html::a("<span class=\"glyphicon glyphicon-check\"></span>", ['/print/index', 'id'=>$dataSet[$naam]['studentid'], 'examenid'=>$examenid, 'onlyWerkproces'=>$thisWp ]);
                        else echo "<div class=\"text-info\">".($value[$thisWp]['status']??0)."</div>";
                    }
                    echo "</td>";
                    echo "<td>&nbsp;</td>";
                    echo "<td>";
